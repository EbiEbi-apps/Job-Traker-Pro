...
const Login: React.FC<LoginProps> = ({ onComplete }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 text-slate-800 dark:text-white transition-colors duration-300">
      <div className="w-full max-w-md bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm rounded-xl shadow-2xl p-10 text-center border border-slate-200 dark:border-slate-700/50">
        <div className="mb-8">
            <h2 className="text-3xl font-bold text-slate-800 dark:text-white mb-2 transition-colors duration-300">{t('welcome.title')}</h2>
            <p className="text-slate-500 dark:text-slate-400 transition-colors duration-300">{t('welcome.subtitle')}</p>
        </div>
        ...
        <button
            onClick={handleLogin}
            className="w-full max-w-sm mx-auto px-6 py-3 rounded-lg bg-slate-100 hover:bg-white font-semibold text-slate-800 shadow-md transition-colors flex items-center justify-center gap-3"
        >
